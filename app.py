...
def donor_list():
    if session['loggedin']==True:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        # cursor.execute('SELECT District FROM Blood_bank WHERE License_Number = %s', (session['License_Number']))
        cursor.execute('SELECT First_name, Last_name, City, Phone_num FROM Donor WHERE Frequent_Donor == "y" AND District = (SELECT District FROM Blood_bank WHERE License_Number = %s', (session['License_Number']))
        rows=cursor.fetchall()
        cursor.close()
